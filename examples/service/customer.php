...
$new = $api->merchant->add('test merchant '. mt_rand(1000, 9999));
$merchant_id = $new['merchant_id'];
$new = $api->offer->add($merchant_id, 'test offer '. mt_rand(1000, 9999));
$offer_id = $new['offer_id'];
$api->merchant->offerAdd($merchant_id, 0 ,$offer_id, 0);


# add new customer
$phone = '7900111' . mt_rand(1000, 9999);
...
# balance change
$response = $api->customer->balanceChange($customer_id, $merchant_id, 1234657, 1000);

Api::r(['balance change' => $response ?: $api->error]);
...
# customer pay card list
$response = $api->customer->paycardList(450);

Api::r(['customer pay card list' => $response ?: $api->error]);
